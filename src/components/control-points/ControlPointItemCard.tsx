    ...
    <Card className={cardClasses} id={`control-point-card-${point.id}`}>
      <CardContent className={cn("flex flex-col p-0", isCurrent ? "gap-2 sm:gap-3" : "gap-0.5 sm:gap-1")}>
        <div className="flex justify-between items-center">
          <span className={cn("text-foreground", isCurrent ? "font-bold text-2xl sm:text-3xl" : "font-semibold text-lg sm:text-xl")}>{point.name}</span>
          <span className={cn("text-foreground font-semibold", isCurrent ? "text-2xl sm:text-3xl" : "text-lg sm:text-xl")}>{displayScheduledTime}</span>
        </div>
        {point.meta && (
          <div className={cn("flex justify-between items-center font-bold", isCurrent ? "text-3xl sm:text-4xl" : "text-2xl sm:text-3xl")}>
            <div>
